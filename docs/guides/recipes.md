...
`agent.lineBlocked` is an **involuntary** block: the platform stopped routing calls to this agent because
their line went busy, unreachable, declined or DND — most often after a call offer rang out unanswered. The
agent did not choose it and nothing in the call flow tells them it happened, so **without this banner an
agent silently stops receiving calls** and has no way to get them back.
...
`resetLineStatus` clears the block on the server; the banner goes away when the next `agentPatch` brings
`lineBlocked: false`. Render the server's state — don't hide the banner locally on click, or a failed reset
looks like a successful one.

Gate on `lineBlocked` alone, never on `presence == BUSY`: a `BUSY` presence the agent chose is voluntary and
needs no Reset. The [state model](../concepts/state-and-events#consuming-it) has the full comparison of the
three availability fields.
