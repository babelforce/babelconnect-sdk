...
**Every code, in full: the [Error codes reference](../protocol/error-codes).** It lists all of them by area —
what each one means, whether it carries a `callId`, and the specific recovery — including the two an agent
meets most often on a rejected outbound call, [`display_as_forbidden` and
`agent_not_available`](../protocol/error-codes#outbound--answering). Codes are stable strings, but handle an
unknown one **generically** (show the `message`, keep the session up): a newer server may add one.
