...
| `transfer_rejected` | a cold transfer's target did not join and the platform named **no reason** the four codes above carry (or none at all). The caller has already been taken off hold and stays with the agent. | ✓ | not fatal, and not a lost call — tell the agent the transfer didn't go through and leave them on it. Branch on the four specific codes first; this is the fallback. |
