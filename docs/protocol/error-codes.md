...
- **`unauthenticated`** is the one code that is about the **session** rather than the command, and
  it overrides the command's own `<intent>_failed` whenever it applies. Branch on it first.

A `message` is always a **sentence written for a person** — the platform's own reason where it gave
one, and the server's own wording where it did not. It is **never a response body**: a platform (or
a proxy in front of it) that answers with an HTML error page, an empty body or a bare 5xx placeholder
gets a sentence from the server naming the status instead. Show the `message`; you never have to
sniff it for markup.
...
| `unsupported_digit` | `sendDigits` carried a character that is not a DTMF key. Only `0`–`9`, `*`, `#` and `A`–`D` (upper case) can be sent as tones; anything else — a letter, `+`, a space, a lower-case `a`–`d` — has no tone to send. | ✓ | **nothing was sent — not the digits before the character, not a substitute tone.** Remove the character and send the burst again. A keypad that offers only the twelve phone keys never produces this. |
