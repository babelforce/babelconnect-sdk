...
The [line-blocked banner recipe](../guides/recipes#a-line-blocked-banner) is the copy-paste version.
It also shows up from the command side: an outbound call placed while the block is up is rejected with
[`agent_not_available`](../protocol/error-codes#outbound--answering) — the same condition, reported on
`onError` instead of in state.
